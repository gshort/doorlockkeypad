...
void setup() {
  ...
  LEDS.addLeds<WS2801, A0, A1, BGR, DATA_RATE_MHZ(1)>(leds, NUM_LEDS); //use pins A0, A1 in software mode...we're only driving 12 LEDs anyhow
  Serial.begin(9600);
  while (!Serial) {
...
void status_idling() { //slowly "breathe" blue, deterministic: call as often as convenient (or possible)
  if(millis() % 10 == 0) {
    LEDS.showColor(CRGB(0, 0, floor((sin(float(millis()) / 4.0) * 0.5 + 1.0) * 255)));
  }
}

void status_reading() { //quickly "breathe" yellow, deterministic: call as often as convenient (or possible)
  if(millis() % 10 == 0) {
    int v = floor((sin(float(millis()) / 2.0) * 0.5 + 1.0) * 255);
